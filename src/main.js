...
import { addWarning, countAtRiskUsers, getAllWarnedUsers, getPendingEscalationCount, getRecentWarnings, getStrikeCount, getWarnings, removeOldestStrike, } from './redis.js';
import { dismissEscalation, isEscalationSubject, parseUsernameFromEscalationSubject, sendEscalationPrompt, sendUserNotification, } from './notifications.js';
...
const warningForm = Devvit.createForm((data) => {
    ...
    await handleWarningSubmit(context, username, postId, postUrl, values.rule, values.severity, values.note || '');
});
const historyForm = Devvit.createForm((data) => ({
    title: data.title,
    description: data.description,
    fields: [],
    acceptLabel: 'Close',
}), async () => { });
async function openWarningForm(context, username, postId, postUrl) {
    const subreddit = await context.reddit.getCurrentSubreddit();
    const config = await loadConfig(context);
    const currentStrikes = await getStrikeCount(context.redis, subreddit.id, username);
...
async function handleWarningSubmit(context, username, postId, postUrl, rule, severity, note) {
    ...
    const newCount = await addWarning(context.redis, subreddit.id, warning, config.expiryDays);
    ...
    try {
        await context.reddit.addModNote({
            ...
            redditId: postId.startsWith('t') ? postId : undefined,
        });
    }
    catch (e) {
    ...
    if (config.notifyUsers) {
        try {
            await sendUserNotification(context, normalizedUsername, subreddit.name, warning, newCount, config);
        }
        catch (e) {
    ...
    if (newCount >= config.threshold) {
        try {
            await sendEscalationPrompt(context, normalizedUsername, subreddit, newCount, config);
        }
        catch (e) {
            console.error('Failed to send escalation:', e);
        }
    }
    context.ui.showToast(`✅ Warning issued to u/${normalizedUsername}. Active strikes: ${newCount}/${config.threshold}`);
}
async function showStrikeHistory(context, username) {
    ...
    const lines = warnings.map((w, i) => {
        ...
        return `${i + 1}. ${date} — ${w.rule} (${w.severity}) ${status}${w.note ? ` | Note: ${w.note}` : ''}`;
    });
    const body = [
...
Devvit.addMenuItem({
    ...
    onPress: async (event, context) => {
        ...
        await openWarningForm(context, post.authorName, event.targetId, `https://reddit.com${post.permalink}`);
    },
});
Devvit.addMenuItem({
    label: '⚠ Issue Warning',
    ...
    onPress: async (event, context) => {
        ...
        await openWarningForm(context, comment.authorName, event.targetId, `https://reddit.com${comment.permalink}`);
    },
});
Devvit.addMenuItem({
    ...
    forUserType: 'moderator',
    onPress: async (event, context) => {
        const postOrComment = event.location === 'post'
...
Devvit.addMenuItem({
    ...
    onPress: async (_event, context) => {
        ...
        const post = await context.reddit.submitPost({
            ...
            preview: (Devvit.createElement("vstack", { alignment: "center middle", height: "100%" },
                Devvit.createElement("text", { size: "large" }, "Loading StrikeManager..."))),
        });
        await context.redis.set(`dashboard:${subreddit.id}`, post.id);
        context.ui.showToast('Dashboard created! Pin this post for your mod team.');
    },
});
Devvit.addCustomPostType({
    ...
    render: (context) => {
        return (Devvit.createElement("blocks", { height: "tall" },
            Devvit.createElement("webview", { url: "index.html", onMessage: async (msg) => {
                    if (msg && typeof msg === 'object' && 'type' in msg && msg.type === 'READY') {
                        const subreddit = await context.reddit.getCurrentSubreddit();
                        const config = await loadConfig(context);
                        const recent = await getRecentWarnings(context.redis, subreddit.id, 50);
                        const atRisk = await countAtRiskUsers(context.redis, subreddit.id);
                        const pending = await getPendingEscalationCount(context.redis, subreddit.id);
                        context.ui.webView.postMessage(JSON.parse(JSON.stringify({
                            type: 'INIT',
                            warnings: recent,
                            atRisk,
                            pending,
...
Devvit.addTrigger({
    ...
    onEvent: async (event, context) => {
        ...
        const messages = Object.values(conversationResponse.conversation?.messages ?? {});
        const currentMessage = messages.find((m) => m.id && event.messageId.includes(m.id));
        ...
        if (isApprove) {
            const duration = config.banDuration > 0 ? config.banDuration : undefined;
            await context.reddit.banUser({
                subredditName: subreddit.name,
                username,
                duration,
...
async function checkAndApplyStrikeDecay(context, subredditId) {
    const config = await loadConfig(context);
    ...
    for (const username of warnedUsers) {
        ...
        if (ageMs >= decayMs) {
            const removed = await removeOldestStrike(context.redis, subredditId, username);
            if (removed) {
                decayCount++;
                const daysSinceOldest = (ageMs / 86400000).toFixed(1);
                console.log(`[StrikeDecay] Removed oldest strike from u/${username} (${daysSinceOldest} days old)`);
...
Devvit.addTrigger({
    ...
    onEvent: async (event, context) => {
        ...
        if (lastStrike && now - parseInt(lastStrike) < 60000) {
            console.log(`[AutoMod Bridge] Cooldown active for u/${targetUsername}, skipping strike (will retry in ${Math.round((60000 - (now - parseInt(lastStrike))) / 1000)}s)`);
            return;
        }
        ...
        try {
            ...
            if (config.notifyUsers) {
                try {
                    await sendUserNotification(context, targetUsername, subreddit.name, warning, newCount, config);
                }
                catch (e) {
            ...
            if (newCount >= config.threshold) {
                try {
                    await sendEscalationPrompt(context, targetUsername, subreddit, newCount, config);
                }
                catch (e) {
                    console.error('Failed to send AutoMod escalation:', e);
                }
            }
            console.log(`[AutoMod Bridge] Strike issued to u/${targetUsername} (${newCount}/${config.threshold})`);
        }
        catch (e) {
