import type { Devvit, Subreddit } from '@devvit/public-api';
import type { SubredditConfig, Warning } from './types.js';
import {
...
export async function sendEscalationPrompt(
  context: Pick<Devvit.Context, 'reddit' | 'redis'>,
  username: string,
  subreddit: Subreddit,
  strikeCount: number,
  config: SubredditConfig
...
export function isEscalationSubject(subject: string | undefined): boolean {
  return (subject ?? '').includes(ESCALATION_SUBJECT_PREFIX);
}

export function parseUsernameFromEscalationSubject(
  subject: string
): string | undefined {
